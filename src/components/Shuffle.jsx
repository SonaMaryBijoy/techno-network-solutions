...
const Shuffle = ({
  ...
  useLayoutEffect(() => {
    ...
    const build = () => {
      ...
      const wordsArr = String(text).split(' ');
      el.innerHTML = '';

      wordsArr.forEach((wordText) => {
        const wordSpan = document.createElement('span');
        wordSpan.className = 'inline-block whitespace-nowrap mr-[0.3em]';

        const charsArr = Array.from(wordText);
        charsArr.forEach(chText => {
          const charSpan = document.createElement('span');
          charSpan.className = 'inline-block';
          charSpan.textContent = chText;
      ...
      const chars = Array.from(el.querySelectorAll('span.inline-block > span'));
      wrappersRef.current = [];
      ...
      chars.forEach(ch => {
        ...
        const h = ch.getBoundingClientRect().height || 24;
  ...
  const baseTw = 'inline-block whitespace-normal break-words will-change-transform font-bold';
